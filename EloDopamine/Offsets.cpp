...
//== CORE ==//
int	Offsets::ViewProjMatrices = 0x3170CF8; //68 ? ? ? ? 51 8B 00
int Offsets::LocalPlayer = 0x314A404; //57 8B 3D ? ? ? ? 3B F7 75
int Offsets::GameTime = 0x3143C44; //F3 0F 11 05 ? ? ? ? 8B 49
int Offsets::ObjectManager = 0x18ADAD8; //89 ? ? ? ? ? 57 C7 06 ? ? ? ? 66 C7 46 04 ? ?
int Offsets::Renderer = 0x31765C0; //8B 0D ? ? ? ? 50 E8 ? ? ? ? 5E 8B // A1 ?? ?? ?? ?? 56 57 BF ?? ?? ?? ?? 8B
//== LISTS ==//
int	Offsets::MissileInterface = 0x314B03C; //8B 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 30 8B 36
int	Offsets::MinionInterface = 0x24FB1E4; //8B 0D ? ? ? ? E8 ? ? ? ? EB 09 // A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 04 85 C0 74 32
int	Offsets::TurretInterface = 0x3142824; //8B 35 ? ? ? ? 8B 76 18 // 8D 44 24 0C 89 35 ? ? ? ? // A1 ? ? ? ? 53 56 8B 70 04
int	Offsets::InhibitorInterface = 0x314B148; //A1 ?? ?? ?? ?? 53 55 56 8B 70 04 8B 40 08
int	Offsets::HeroInterface = 0x18ADB74; //89 44 24 18 A1 ? ? ? ? 53 // 8B 15 ? ? ? ? 0F 44 C1
int	Offsets::BuildingInterface = 0x1859378;
int	Offsets::oGetHPBarPos = 0x30C9;  //E8 ?? ?? ?? ?? 8B 4E ?? 8D 54 ?? ?? 52 8B 01 FF ?? ?? 5E 83 ?? ?? C3
int	Offsets::AINetClient = 0x314FF34; //8B 0D ?? ?? ?? ?? 85 C9 74 06 8B 01 6A 01 FF 10 8B 0D ?? ?? ?? ?? C7 05 ?? ?? ?? ?? ?? ?? ?? ?? 85 C9 74 06 8B 01 6A 01 FF 10 C3 //C7 05 ? ? ? ? ? ? ? ? 85 C9 74 06 8B 01 6A 01 FF 10 C3 - //A1 ?? ?? ?? ?? 88 4C 24 13
int	Offsets::CpuInstance = AINetClient - 0x3C;
//== MINIMAP STRUCTURE ==//
int	Offsets::MinimapObject = 0x3143C88; //8B 1D ? ? ? ? 85 DB 74 19 //74 22 8B 0D ? ? ? ? 85 C9 74 18 80 79 38 00
int	Offsets::MinimapObjectHud = 0x15C;  //C7 44 24 ? ? ? ? ? E8 ? ? ? ? 8B B7 ? ? ? ? 8B //Tomar primer valor ejemplo [esp+120h+var_D4]
int	Offsets::MinimapHudPos = 0x3C;
int	Offsets::MinimapHudSize = 0x44;
//== CHAT STRUCTURE ==//
int	Offsets::Chat = 0x314B094; //8B 0D ? ? ? ? 8A D8 85 // 8B 0D ? ? ? ? 8A D8 85 C9 //8B 0D ? ? ? ? 83 78 44 00
int	Offsets::ChatIsOpen = 0x6BC; //C7 86 ? ? ? ? ? ? ? ? E8 ? ? ? ? 83 C4 04 85 C0 75 30 F6 86 ? ? ? ? ? 75 1B 38 86 ? ? ? ?
//== BUFF MANAGER STRUCTURE ==//
int	Offsets::ObjBuffManager = 0x2338; //8D 83 ? ? ? ? 50 8D AB ? ? ? ? // //8b 81 ? ? ? ? 81 c1 ? ? ? ? 8b 40 0c ff e0 cc // 4 first characters are the offset
...
int	Offsets::AIManager = 0x2E8C; //LPLAYER->VTABLE 149 MOV
...
int	Offsets::ObjMaxHealth = 0xE8C;
int	Offsets::ObjAbilityHaste = 0x16A8;
int	Offsets::ObjMagicPenFlat = 0x118C;
int	Offsets::ObjAbilityPower = 0x1758;
int	Offsets::ObjMagicPen = 0x1278;
int	Offsets::ObjMagicPenMulti = 0x1280;
int	Offsets::ObjArmorPen = ObjMagicPen + 0x4;
int	Offsets::ObjMagicPenMod = ObjMagicPen + 0x8;
int	Offsets::ObjLethality = 0x1294;
int	Offsets::ObjArmor = 0x1384;
int	Offsets::ObjBonusArmor = 0x1388;
int	Offsets::ObjMagicRES = 0x138C;
int	Offsets::ObjBonusMagicRES = 0x1390;
int	Offsets::ObjBaseATK = 0x135C;
int	Offsets::ObjBonusATK = 0x12D4;
int	Offsets::ObjBonusAP = ObjBonusATK + 0x10;
int	Offsets::ObjBonusAPMOD = ObjBonusAP + 0x4;
int	Offsets::ObjATTACKSPEEDMOD = 0x1268;
int	Offsets::ObjMOVESPEED = 0x139C;
int	Offsets::ObjSpellBOOK = 0x29C8; //8B 84 83 ? ? ? ? EB 06 8B 83 ? ? ? ? 85 C0 0F 84 ? ? ? ? 53 8B CF E8 ? ? ? ? 8B C8 8B 10 FF 52 18 8B F0
int	Offsets::ObjSpellBOK2 = 0x2520; //ObjSpellBOOK+0x20
int	Offsets::ObjTRANSFORMATION = 0x3040;
int	Offsets::ObjName = 0x2DB4;
int	Offsets::ObjLVL = 0x35A4; // ó 0x353C
int	Offsets::ObjSIZEMULTIPLIER = 0x1838;
int	Offsets::ObjEXPIRY = 0x298;
int	Offsets::ObjCritMOD = ObjBaseATK + 0x10;
int	Offsets::ObjCrit = 0x1858;//ObjCritMOD + 0x10;
int	Offsets::ObjCritMULTI = 0x12B8;
int	Offsets::ObjATKSpeedMulti = 0x1358; //F3 0F 10 80 ? ? ? ? F3 0F 11 ? ? 50 E8
int	Offsets::ObjATKRange = 0x13A4; //D9 83 ? ? ? ? 5F 5E 5B 83 C4 08 C3
int	Offsets::ObjTargetable = 0xD04;
int	Offsets::ObjInvulnerable = 0x029C;
int	Offsets::ObjIsMoving = 0x34E9;
int	Offsets::ObjDirection = 0x1BE8;
int	Offsets::ObjItemList = 0x35F0;
int	Offsets::ObjExperience = 0x3384;
int	Offsets::ObjAdditionalAPMulti = 0x12E4;
...
////CHARDATA///
int	Offsets::ObjPreCharData = 0x2D8C;
